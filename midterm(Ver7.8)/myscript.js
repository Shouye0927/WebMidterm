...
if (sessionStorage.getItem('reloaded') != null) {
    ...
    .then(response => {
        if (response.ok) {
            ...
            window.location.replace("http://localhost/midterm(ver7.2)/index.php");
        } else {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const urlParams = new URLSearchParams(window.location.search);
              if (urlParams.get('error') === 'no_results') {
    ...
    const current_theme = saved_theme || default_theme;
    ...
    window.likeBlog = function likeBlog(blogId) {
        ...
        .then(data => {
            if (data.success) {
                ...
                alert('Failed to like the blog: ' + data.message);
            }
        })
        // .catch(error => {
